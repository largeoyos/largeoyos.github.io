...
个人静态网站/知识库，同时部署在 GitHub Pages 和 Vercel 上。包含博客系统、AI 教程文库、网页小游戏、快捷导航页面。

- **站点**: https://largeoyos.github.io
- **语言**: 中文为主
- **技术栈**: 原生 HTML/CSS/JS，部分子项目使用 React + Vite + TypeScript
- **托管**: GitHub Pages（已配置 `.nojekyll`，禁止删除）+ Vercel
- **Vercel 可用能力**: 静态站 CDN 托管、自动 HTTPS、自定义域名、Git 推送自动部署、分支/PR 预览部署、回滚到历史部署、访问分析与性能指标、通过 `vercel.json` 配置 redirects/rewrites/headers。
- **Vercel 扩展能力**: 如后续需要动态功能，可增加 Serverless Functions / Edge Functions、API 路由、环境变量、定时任务等；当前仓库主体仍按纯静态站维护，除非用户明确要求，不主动引入 Vercel 专属后端能力。
...
### Dual Deployment
- 本站同时部署到 GitHub Pages 与 Vercel。默认改动必须保持两边都能正常工作。
- 静态页面、Markdown 渲染、游戏与工具页优先使用相对路径和纯前端实现，避免把基础浏览体验绑定到某一个平台。
- 可以利用 Vercel 的预览部署、回滚、headers/redirects/rewrites、Serverless/Edge Functions 等能力，但引入 Vercel-only 功能前需明确说明取舍，并确认不会破坏 GitHub Pages 版本，或说明该功能只在 Vercel 生效。
