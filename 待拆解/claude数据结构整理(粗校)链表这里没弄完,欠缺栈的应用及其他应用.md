...
$$ASL_{成功}=\frac{1\times1+2\times2+3\times4}{7}=\frac{17}{7}\approx2.43.$$
...
$$N(0)=0,\quad N(1)=1,\quad N(2)=2,\quad N(h)=N(h-1)+N(h-2)+1.$$
...
**类比**：AVL 树每次只判断“左还是右”，而 B 树每次判断落入哪一段。树高从 $O(\log_2 n)$ 降到 $O(\log_m n)$（$m$ 为阶数），因此磁盘 I/O 次数显著减少。
...
**结论**：B 树高度为 $O(\log_m n)$；$m$ 越大，树越矮，I/O 次数通常越少。
...
**关于 `INF = 0x3f3f3f3f`：** 它足够大，而且 `INF + INF` 仍不超过 32 位有符号 `int` 上限。若使用 `INT_MAX`，必须在相加前判断距离是否有限，避免有符号整数溢出。
...
**关键观察**：每调用一次内层 DFS/BFS（`DFS_M` 或 `BFS_M`），就会遍历一个连通分量。
...
$C_1=0,C_2=0,C_3=2,C_4=1,C_5=1,C_6=3$。
...
第三，**边权相加可能溢出**。执行 `dist[u] + weight` 前必须判断 `dist[u] != INF`，并确保结果不超过类型上限。
...
**易错五：无穷大值的选择。** 不要直接对 `INT_MAX` 做加法；应使用有限哨兵值、宽整数类型或显式溢出检查。
...
**C++ `std::stable_sort`**：标准保证排序稳定性和复杂度要求，但不规定必须使用归并排序。
